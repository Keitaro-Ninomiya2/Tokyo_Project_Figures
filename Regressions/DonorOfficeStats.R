...
#
# Compares offices (ka x position) that had transfers OUT vs. those that
# did not, conditional on kyoku x position FE.
#
...
# A worker transferred out of (office_id, ka, pos_norm) in year t
# if they were at that unit in year t-1 but moved to a different office/ka in year t
...
# Transfers OUT: only count workers who moved to a DIFFERENT kyoku
...
# Add new hire and promotion counts
transition_agg <- worker_transitions %>%
...
panel <- panel %>%
  ...
  left_join(transfers_out, by = c("office_id", "ka", "pos_norm", "year_num")) %>%
  mutate(
...
panel_valid <- panel %>% filter(!is.na(ka), !is.na(norm_kyoku))

cat("Panel:", nrow(panel_valid), "obs\n")
cat("  Donor (transfers out > 0):", sum(panel_valid$is_donor == 1), "\n")
cat("  Non-donor:", sum(panel_valid$is_donor == 0), "\n")
...
results_uncond <- map2_dfr(vars, var_labels, function(v, lab) {
  d1 <- panel_valid %>% filter(is_donor == 1) %>% pull(!!sym(v))
  d0 <- panel_valid %>% filter(is_donor == 0) %>% pull(!!sym(v))
...
run_spec <- function(v, fe_str, size_var = "log_n_workers") {
  if (v == "n_workers") {
    fml <- as.formula(paste0(v, " ~ is_donor | ", fe_str))
  } else {
    fml <- as.formula(paste0(v, " ~ is_donor + ", size_var, " | ", fe_str))
  }
  tryCatch({
    m <- feols(fml, data = panel_valid, cluster = ~office_id)
    tibble(
...
results_office <- map2_dfr(vars, var_labels, function(v, lab) {
  bind_cols(tibble(Variable = lab), run_spec(v, "ka + pos_norm + year_num"))
})

results_dept <- map2_dfr(vars, var_labels, function(v, lab) {
  bind_cols(tibble(Variable = lab), run_spec(v, "norm_kyoku + pos_norm + year_num"))
})

cat("\nOffice (Ka) FE:\n"); print(results_office, width = Inf)
cat("\nDept (Kyoku) FE:\n"); print(results_dept, width = Inf)

n_clusters_donor <- n_distinct(panel_valid$office_id)
cat("Number of clusters (offices):", n_clusters_donor, "\n")
...
fmt_row <- function(res, i) {
  ...
  list(
    ...
    n    = if (is.na(r$N))    "---" else format(r$N, big.mark = ",")
  )
}
...
for (i in 1:nrow(results_office)) {
  ro <- fmt_row(results_office, i)
  rd <- fmt_row(results_dept, i)
  tex_lines <- c(tex_lines,
    paste0(results_office$Variable[i], " & ", ro$coef, " & ", rd$coef, " \\\\"),
    paste0(" & ", ro$se, " & ", rd$se, " \\\\")
  )
}
...
tex_lines <- c(tex_lines,
  "\\midrule",
  paste0("Observations & ",
         format(results_office$N[1], big.mark = ","), " & ",
         format(results_dept$N[1],   big.mark = ","), " \\\\"),
  "\\bottomrule",
  "\\end{tabular}",
  "",
  "\\begin{tablenotes}[flushleft]",
  "\\footnotesize",
  paste0("\\item \\textit{Notes:} Sample: ka $\\times$ position $\\times$ year (1938--1945). ",
         "A \\emph{donor} office is one that had at least one worker transfer out in that year. ",
         "Panel~A reports unconditional means. Panel~B reports coefficients from OLS regressions ",
         "on a donor indicator, with position and year fixed effects plus either office (ka) or ",
         "department (kyoku) fixed effects, controlling for log office headcount. ",
         "Standard errors clustered at the office level (", n_clusters_donor, " clusters). ",
         "$^{***}p<0.01$, $^{**}p<0.05$, $^{*}p<0.1$."),
  "\\end{tablenotes}",
  "\\end{threeparttable}",
  "\\end{table}"
