################################################################################
# 1944 size-control diagnostic for the cleaned slack threshold result.
# Adds destination bureau size and total nearby candidate-pool controls to test
# whether matched same-section slack survives non-mechanical size variation.
################################################################################
...
prewar_arrivals <- df_names %>%
  ...
  mutate(
    arrival_type = arrival_distance(kyoku_group_filled, ka_group_filled, ka, lag_kyoku_group, lag_ka_group, lag_ka),
    is_transfer_in = !is.na(lag_office_id) & lag_office_id != office_id
  )

prewar_diffdept_pairs <- prewar_arrivals %>%
  filter(is_transfer_in, arrival_type == "different_department", !is.na(kyoku_group_filled), !is.na(lag_kyoku_group)) %>%
  transmute(kyoku_a = pmin(kyoku_group_filled, lag_kyoku_group), kyoku_b = pmax(kyoku_group_filled, lag_kyoku_group)) %>%
...
dest_kyoku_size_1944 <- df_names %>%
  filter(year_num == analysis_year, !is.na(norm_kyoku_filled)) %>%
  group_by(norm_kyoku_filled, year_num) %>%
  summarise(dest_kyoku_workers = n(), .groups = "drop")
...
drafted_profiles_1944 <- df_all %>%
  ...
  group_by(section_id_filled, office_id, year_num, ka, ka_group_filled, norm_kyoku_filled, kyoku_group_filled, pos_norm, occupation, pos_rank) %>%
  summarise(n_drafted_profile = n(), .groups = "drop")

drafted_section_totals_1944 <- drafted_profiles_1944 %>%
  group_by(section_id_filled, office_id, year_num, ka, ka_group_filled, norm_kyoku_filled, kyoku_group_filled) %>%
...
# Total nearby pools, regardless of exact position match.
pool_matches_1944 <- drafted_section_totals_1944 %>%
  rename(
  ...
  inner_join(candidate_workers_1944, by = "year_num", relationship = "many-to-many") %>%
  filter(lag_office_id != dest_office_id) %>%
  mutate(
    pool_distance = arrival_distance(dest_kyoku_group, dest_ka_group, dest_ka, lag_kyoku_group, lag_ka_group, lag_ka),
  ...
  filter(pool_distance != "different_department" | prewar_pair_allowed == 1L) %>%
  filter(!is.na(pool_distance)) %>%
  distinct(dest_section_id, year_num, staff_id, pool_distance)

pool_counts_1944 <- pool_matches_1944 %>%
  count(dest_section_id, year_num, pool_distance, name = "n_pool") %>%
  pivot_wider(names_from = pool_distance, values_from = n_pool, values_fill = 0, names_prefix = "pool_")
...
slack_matches_1944 <- drafted_profiles_1944 %>%
  ...
  inner_join(candidate_workers_1944,
             by = c("year_num" = "year_num", "dest_pos_norm" = "lag_pos_norm", "dest_occupation" = "lag_occupation"),
             relationship = "many-to-many") %>%
  filter(lag_office_id != dest_office_id, abs(lag_pos_rank - dest_pos_rank) <= 1) %>%
  mutate(
    slack_distance = arrival_distance(dest_kyoku_group, dest_ka_group, dest_ka, lag_kyoku_group, lag_ka_group, lag_ka),
    kyoku_a = pmin(dest_kyoku_group, lag_kyoku_group),
    kyoku_b = pmax(dest_kyoku_group, lag_kyoku_group)
  ) %>%
  left_join(prewar_diffdept_pairs, by = c("kyoku_a", "kyoku_b")) %>%
...
slack_counts_1944 <- slack_matches_1944 %>%
  ...
  pivot_wider(names_from = slack_distance, values_from = n_slack, values_fill = 0, names_prefix = "slack_")

panel_1944 <- drafted_section_totals_1944 %>%
  left_join(outcomes_section_1944, by = c("section_id_filled", "office_id", "year_num", "ka", "ka_group_filled", "norm_kyoku_filled", "kyoku_group_filled")) %>%
  left_join(section_year_panel_1944 %>% select(section_id_filled, office_id, year_num, n_workers), by = c("section_id_filled", "office_id", "year_num")) %>%
  left_join(dest_kyoku_size_1944, by = c("norm_kyoku_filled", "year_num")) %>%
  left_join(slack_counts_1944, by = c("section_id_filled" = "dest_section_id", "year_num")) %>%
  left_join(pool_counts_1944, by = c("section_id_filled" = "dest_section_id", "year_num")) %>%
  mutate(
    across(c(transfers_in, new_hires, slack_same_section, slack_same_department, slack_different_department,
             pool_same_section, pool_same_department, pool_different_department),
           ~replace_na(., 0)),
    transfer_share = if_else(transfers_in + new_hires > 0, transfers_in / (transfers_in + new_hires), NA_real_),
    ...
    log_pool_same_section = log(pool_same_section + 1),
    log_pool_same_department = log(pool_same_department + 1),
    log_pool_different_department = log(pool_different_department + 1),
    log_section_size = log(n_workers + 1),
    log_dest_kyoku_size = log(dest_kyoku_workers + 1)
  ) %>%
  filter(n_drafted_male > 0, !is.na(norm_kyoku_filled))

run_model <- function(outcome, add_size_controls = FALSE) {
  rhs <- c(
  ...
  if (add_size_controls) {
    rhs <- c(
      rhs,
      "log_dest_kyoku_size",
      "log_pool_same_section",
      "log_pool_same_department",
      "log_pool_different_department"
    )
  }
  fml <- as.formula(paste0(outcome, " ~ ", paste(rhs, collapse = " + "), " | norm_kyoku_filled"))
  feols(fml, data = panel_1944, cluster = ~office_id)
}

models <- list(
  baseline_transfers = run_model("transfers_in", add_size_controls = FALSE),
  sizectrl_transfers = run_model("transfers_in", add_size_controls = TRUE),
  baseline_share = run_model("transfer_share", add_size_controls = FALSE),
  sizectrl_share = run_model("transfer_share", add_size_controls = TRUE)
)

results <- imap_dfr(models, function(mod, spec) {
  tidy(mod) %>%
    filter(term %in% c(
      "any_slack_same_section",
      "log_slack_same_section",
    ...
    mutate(specification = spec, n = nobs(mod))
})

write_csv(results, file.path(result_dir, "tokyoto_1944_size_control_diagnostic_results.csv"))

cat("1944 size-control diagnostic:\n")
print(results, n = Inf)
