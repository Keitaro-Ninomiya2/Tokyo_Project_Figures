################################################################################
# Descriptive split of same-section selected transfers into:
# - same kakari
# - same ka, different kakari
# using the refined 1944 office-level occ-rank selection sample.
################################################################################
...
selected_workers <- worker_arrivals_1944 %>%
  ...
  inner_join(
    office_choice %>%
      select(event_id, year_num, dest_office_id, dest_section_id, dest_pos_norm, dest_occupation,
             dest_pos_rank, origin_office_id, distance_cat),
    by = c(
      "year_num",
  ...
  filter(distance_cat == "same_section") %>%
  mutate(
    kakari_relation = case_when(
      is.na(kakari) | is.na(lag_kakari) ~ "missing_kakari",
      kakari == lag_kakari ~ "same_kakari",
      TRUE ~ "same_ka_diff_kakari"
    )
  )

summary_table <- selected_workers %>%
  count(kakari_relation, sort = TRUE, name = "n_selected_workers") %>%
  mutate(share = n_selected_workers / sum(n_selected_workers))

event_level_summary <- selected_workers %>%
  group_by(event_id) %>%
  summarise(
    any_same_kakari = any(kakari_relation == "same_kakari"),
    any_diff_kakari = any(kakari_relation == "same_ka_diff_kakari"),
    all_missing = all(kakari_relation == "missing_kakari"),
    .groups = "drop"
  ) %>%
  summarise(
    n_same_section_events = n(),
    share_any_same_kakari = mean(any_same_kakari),
    share_any_diff_kakari = mean(any_diff_kakari),
    share_all_missing_kakari = mean(all_missing)
  )

write_csv(summary_table, file.path(result_dir, "vacancy_selection_1944_same_section_kakari_split.csv"))
write_csv(event_level_summary, file.path(result_dir, "vacancy_selection_1944_same_section_kakari_event_summary.csv"))

cat("Same-section selected-worker split:\n")
print(summary_table, n = Inf)
cat("\nSame-section event summary:\n")
print(event_level_summary)
